// dsh-auto-vision v0.3 —— 自动视觉路由插件（零依赖，仅用 cordis ctx 事件）
//
// 监听 agent/request / agent/request-error：
//   1) 会话历史出现图片块 / read_image 调用 / 视觉意图时，把 DeepSeek 路由切到视觉模型；
//   2) 用户消息一进来就预判视觉需求（链接+分析动词 / 图片名词 / 视觉动作），
//      提前切 vision —— 解决"首次主动调用 read_image 时历史还没图"的时序问题。
//   3) 切换后致命失败自动回退原模型一次。
//
// v0.3 相对 v0.2 的增强：
//   - 用户消息"主动预判"：含链接+分析/视觉动词、或图片类名词、或视觉动作 → 立即切 vision
//   - 覆盖"发公众号/网页链接、给图片任务"时马上能读图的场景（无需先失败再切）
//
...
//   intentPattern:    string  自定义预判正则（覆盖内置词表）
...
// —— 用户消息"主动预判"（v0.3 新增）——
// 收到用户消息即判断是否可能含图：链接+分析动词，或图片/视觉类名词，或直接视觉动作。
function userNeedsVision(text) {
...
function eventVerdict(ev, historyPattern, intentEnabled) {
  ...
  switch (ev.type) {
    case "user/message": {
      ...
      if (intentEnabled) {
        ...
        if (userNeedsVision(text)) return { needs: true, reason: "intent" };
      }
      return { needs: false };
    }
...
function apply(ctx, config) {
  ...
  ctx.on("agent/request", async (payload, next) => {
    ...
    const verdict = Array.isArray(events) ? scanEvents(events, historyPattern, intentEnabled) : { needs: false, reason: null };
